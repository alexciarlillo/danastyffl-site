<template>
    <div class="matchup">
        <v-touch class="header bg-mfl-blue fixed w-full md:relative" v-on:swipeleft="nextMatchup" v-on:swiperight="prevMatchup">
            <div class="flex justify-between items-stretch">
                <div class="franchise-header flex-1 text-center flex flex-col justify-between p-2">
                    <div class="h-full flex items-center justify-center">
                        <span class="text-sm lg:text-lg font-header text-grey">{{ getFranchiseName(league, awayFranchise.id) }}</span>
                    </div>
                    <div class="text-base lg:text-xl mt-1 font-header text-grey font-semibold">{{ awayFranchise.score }}</div>
                </div>
                ...
                <div class="franchise-header flex-1 text-center flex flex-col justify-between p-2">
                    <div class="h-full flex items-center justify-center">
                        <span class="text-sm lg:text-lg font-header  text-grey">{{ getFranchiseName(league, homeFranchise.id) }}</span>
                    </div>
                    <div class="text-base lg:text-xl mt-1 font-header text-grey font-semibold">{{ homeFranchise.score }}</div>
                </div>
            </div>
        ...
        <div class="flex scores bg-grey-lightest" v-bind:style="{ paddingTop: topPadding + 'px' }">
            ...
            <div class="positions flex-shrink w-6">
                <div v-for="(position, index) in positions" class="bg-grey-lighter h-12 flex justify-center items-center border-b-2 border-grey-lightest" :key="index">
